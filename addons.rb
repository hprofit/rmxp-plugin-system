...
# order of hash keys is not guaranteed.  This change modifies the Hash#encode_with method
# to sort the hash based on key before emitting the YAML for it.
#-------------------------------------------------------------------------------------
class Hash
  # Replacing the encode_with function so it'll serializes hashes sorted (by their keys)
  def to_yaml( opts = {} )
